...
void SkyboxGeneratePrefilteredMap(MFSkybox* skybox, MFSkyboxConfig config, MFRenderer* renderer) {
    ...
    for(u32 i = 0; i < maxMipLevels; i++) {
        ...
        info.format = skybox->isHdr ? MF_FORMAT_R32G32B32A32_SFLOAT : MF_FORMAT_R8G8B8A8_UNORM;
        info.usage = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
        VulkanImageCreate(&tempImage[i], info);
    }
    // Pipeline
    {
        VkVertexInputBindingDescription binding = {
    ...
    // Main recording
    {
        ...
        for(u32 mip = 0; mip < maxMipLevels; mip++) {
            u32 mipSize = cubemapImage->info.width * pow(0.5, mip);
            ...
            for(u32 i = 0; i < 6; i++) {
                ...
                vkCmdPushConstants(cmdBuff, pipeline.layout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(MFMat4) + sizeof(f32), &pcData);
                VulkanBuffer* vertBuffer = (VulkanBuffer*)mfGpuBufferGetBackend(skybox->mesh.vertBuffer);
                VulkanBuffer* indexBuffer = (VulkanBuffer*)mfGpuBufferGetBackend(skybox->mesh.indBuffer);
                VkDeviceSize offsets[1] = {0};
                vkCmdBindVertexBuffers(cmdBuff, 0, 1, &vertBuffer[0].handle, offsets);
                ...
                // Copying to cubemap
                {
                    ...
                    vkCmdCopyImage(cmdBuff, tempImage[mip].image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, cubemapImage->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy);
                }
            }
        }
