    ...
    // Resource sets
    {
        ...
        for(u64 k = 0; k < state->scene.meshCompPool.len; k++) {
            ...
            for(u64 i = 0; i < component->model.meshCount; i++) {
                ...
                MFGpuImage* metallicRoughnessImage = mfMaterialSystemGetImageFromArray(MF_MODEL_MAT_TEXTURE_METALNESS, &state->materialImages[k], &component->model, 0, appState->renderer);
...
static void ConfigModelImages(MFTState* state, MFDefaultAppState* appState) {
    ...
    for(u64 i = 0; i < state->entityCount; i++) {
        ...
        for(u64 k = 0; k < component->model.meshCount; k++) {
            ...
            image = mfMaterialSystemGetImageFromArray(MF_MODEL_MAT_TEXTURE_LIGHTMAP, &state->materialImages[i], &component->model, k, appState->renderer);
            mfGpuImageSetBinding(image, 6);
        }
